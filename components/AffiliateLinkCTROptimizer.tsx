...
export default function AffiliateLinkCTROptimizer() {
  ...
  const [surroundingContentQuality, setSurroundingContentQuality] =
    useState<number>(5);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <label
            htmlFor="surroundingContentQuality"
            className="block text-sm font-medium text-gray-300"
          >
          ...
          <input
            ...
            onChange={(e) =>
              setSurroundingContentQuality(Number(e.target.value))
            }
            className="mt-1 block w-full"
          />
          <span className="text-sm text-gray-400">
            {surroundingContentQuality}
          </span>
        </div>
      </motion.div>
      <motion.div
      ...
      >
        ...
        <p className="text-sm text-gray-400 mt-2">
          ...
          {(((Number(optimizedCTR) - currentCTR) / currentCTR) * 100).toFixed(
            2
          )}
          %
        </p>
      ...
      >
        <p>
          This optimizer provides estimates based on general best practices.
          Actual results may vary depending on your specific niche, audience,
          and content.
        </p>
        <p className="mt-2">
          {`Want to learn advanced techniques for optimizing your affiliate links? Check out John Crestani's`}{" "}
